...
private const val UNLOCK_SELECTION_DELAY = 400L
private const val DEFAULT_LENGTH = 100
private const val MIN_LENGTH = 4
private const val UNIQUE_NAME_MIN_LENGTH = 2
private const val MAX_LENGTH = 26

/**
 * EditText с подсветкой уникальных имен и выдачей уникального имени по коллбеку
 * */
open class MeeraEditTextExtended : AppCompatEditText {

    private var friendsTextColorRes = R.color.map_friend_A6C90D
    private var defaultTextColorRes = R.color.uiKitColorForegroundInvers
    ...
    private val updateEndTextWatcher: TextWatcher = object : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable?) {
            ...
            unlockSelectionListenerTask = Timer("UnlockSelectionListener", false).schedule(UNLOCK_SELECTION_DELAY) {
    ...
    public fun setDefaultTextColor(@ColorRes colorRes: Int) {
        defaultTextColorRes = colorRes
    }
     ...
     * */
    public fun setCheckUniqueNameStrategy(newStrategy: MeeraEditTextExtended.CheckUniqueNameStrategy) {
    ...
    private fun highlightUsingRegex() {
        text?.let { regexGetAllWords.findAll(it) }?.filter { it.value.isNotEmpty() }?.forEach { matchResult ->
            ...
            if (isHighlightableUniqueName(charSequence)) {
                text?.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(context, friendsTextColorRes)),
                ...
                text?.setSpan(
                    ForegroundColorSpan(ContextCompat.getColor(context, defaultTextColorRes)),
                    matchResult.range.first,
                    matchResult.range.last + 1,
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE
                )
    ...
    private fun getHashtagBeforePointer(): String? {
        ...
        if (specialCharLastIndex >= 0 && specialCharLastIndex < textBeforeCursor.length) {
            val hashtag = textBeforeCursor
                .substring(specialCharLastIndex, textBeforeCursor.length)
                .takeIf { it.length <= DEFAULT_LENGTH }
                ?.trimStart()
    ...
    public class CheckUniqueNameStrategyAddPost : BaseCheckUniqueNameStrategy() {
        override fun checkForSearch(charSequence: CharSequence?): Boolean {
            return charSequence
                ...
                ?.let { text: String ->
                    text.matches(uniqueNameRegex)
                        && text.length >= UNIQUE_NAME_MIN_LENGTH // @ + 1 символ
                        && text.length <= MAX_LENGTH
                        && !text.contains("..")
                        && !text.endsWith(".")
                        && text.filter { it == '@' }.length == 1
