...
internal class MediaViewerPreview @JvmOverloads constructor(
        ...
        set(value) {
            ...
            if (selectedMediaCount > 0) {
                ...
                when (selectedMediaCount) {
                    ...
                    in 2..4 -> {
                        tvContinue?.text = resources.getString(R.string.add_few_photo, selectedMediaCount)
                    }

                    5 -> {
                        tvContinue?.text = resources.getString(R.string.add_many_photo, selectedMediaCount)
                    }
                }
            } else {
    ...
    init {
        View.inflate(context, R.layout.image_viewer_preview_layout, this)
        ...
        cvAddPhotoBtn?.setOnClickListener {
            if (!isPressAddPhoto) {
                ...
                images?.let {
                    if (it.isEmpty()) it.add(Uri.parse(imagesAdapter?.getImgUrlByPosition(currentPosition)))
        ...
        ivOverlayMenu?.setOnClickListener {
            if (menu == null) {
                menu = MenuPopup(context).apply {
                    ...
                    show(ivOverlayMenu, Gravity.TOP or Gravity.END, 0, -(ivOverlayMenu?.height ?: 0))
                }
            } else if (!menu!!.isShowing) {
                menu?.show(ivOverlayMenu, Gravity.TOP or Gravity.END, 0, -(ivOverlayMenu?.height ?: 0))
            }
        }
    }
            ...
            mediaViewerPhotoEditorCallback?.onOpenPhotoEditor(
                ...
                resultCallback = object : MediaViewerPhotoEditorCallback.MediaViewerPhotoEditorResultCallback {
                    ...
                    override fun onVideoReady(resultUri: Uri) {
                        Log.e("MediaViewerPreview", "On video Ready URI:$resultUri")
                    }
