...
import com.noomeera.nmravatarssdk.ui.adapters.CategoriesAdapter
import com.noomeera.nmravatarssdk.ui.adapters.CategoriesItem
import com.noomeera.nmravatarssdk.ui.adapters.ColorsAdapter
import com.noomeera.nmravatarssdk.ui.adapters.ColorsItem
import com.noomeera.nmravatarssdk.ui.adapters.ItemsAdapter
import com.noomeera.nmravatarssdk.ui.view.CustomSwitch
import java.io.File
import java.util.concurrent.atomic.AtomicInteger

class MeeraAvatarEditorFragment(override val layout: Int = R.layout.fragment_editor_main) : MeeraBaseFragment() {
    ...
    private fun generateCategoriesItems(woman: Boolean): List<CategoriesItem> {
        ...
        return listOfNotNull(
            CategoriesItem(
                R.string.PART_SELECTOR_HEAD, R.drawable.ic_head, R.drawable.ic_head_selected, assetCategories.head
            ), CategoriesItem(
                R.string.PART_SELECTOR_HAIR, R.drawable.ic_hair, R.drawable.ic_hair_selected, assetCategories.hair
            ), CategoriesItem(
                R.string.PART_SELECTOR_EYES, R.drawable.ic_eyes, R.drawable.ic_eyes_selected, assetCategories.eyes
            ), CategoriesItem(
                ...
                R.string.PART_SELECTOR_NOSE, R.drawable.ic_nose, R.drawable.ic_nose_selected, assetCategories.nose
            ), CategoriesItem(
                R.string.PART_SELECTOR_MOUTH, R.drawable.ic_mouth, R.drawable.ic_mouth_selected, assetCategories.mouth
            ), if (woman) null else CategoriesItem(
                R.string.PART_SELECTOR_BEARD, R.drawable.ic_beard, R.drawable.ic_beard_selected, assetCategories.beard
            ), CategoriesItem(
                R.string.PART_SELECTOR_SHIRT, R.drawable.ic_shirt, R.drawable.ic_shirt_selected, assetCategories.shirt
            ), CategoriesItem(
                R.string.PART_SELECTOR_HAT, R.drawable.ic_hat, R.drawable.ic_hat_selected, assetCategories.hat
            ), CategoriesItem(
    ...
    private fun copyLastStateWithAsset(
        typeResource: Int, avatarStateAsset: AvatarStateAsset
    ): AvatarState {
        ...
        return when (typeResource) {
            ...
            R.string.PART_SELECTOR_EYEBROWS -> lastState.copy(
                assets = lastState.assets.copy(eyebrows = avatarStateAsset)
            )
            ...
            R.string.PART_SELECTOR_BACKGROUND -> lastState.copy(
                assets = lastState.assets.copy(background = avatarStateAsset)
            )

            R.string.PART_SELECTOR_ACCSESSORIES -> lastState.copy(
                assets = lastState.assets.copy(accessories = avatarStateAsset)
            )

            R.string.PART_SELECTOR_HAT -> lastState.copy(
                assets = lastState.assets.copy(hat = avatarStateAsset)
            )
    ...
    private fun update() {
        try {
            ...
            assetsConfig.allAssetSets().find { it.id == selectedAvatarStateAsset.id }?.let { assetSet ->
                ...
                if (assetSet.variants == null) colorsAdapter.submitList(listOf())
                selectedAvatarStateAsset.color?.apply {
                    colorsAdapter.selectedColorId = this
                }
                binding.vColorSelector.visibility = if (assetSet.variants == null) View.GONE else View.VISIBLE
            }
            ...
            computationThreadPool.execute {
                ...
                binding.vAvatarView.post {
                    if (oldUpdatesCount == updatesCount.get()) binding.vAvatarView.setAssets(assets)
                }
            }
        } catch (e: Error) {
            e.printStackTrace()
    ...
    private fun moveCategorySelector(forward: Boolean) {
        ...
        if (newIndex > categories.size - 1) newIndex = categories.size - 1
    ...
    private fun init() {
        ...
        with(binding.fragmentEditorOverlay) {
            vGenderSwitch.checkedChangeListener = object : CustomSwitch.CustomSwitchCheckedChangeListener {
...
//        vPartSelector.recycledViewPool.setMaxRecycledViews(0, 0)
        ...
        colorsAdapter = ColorsAdapter(requireContext()) {
            ...
            val selectedAvatarStateAsset = assetForCategory(categoriesAdapter.selectedCategoryTypeResource)

            val avatarStateAsset = AvatarStateAsset(
                selectedAvatarStateAsset.id, it.color.id
            )
            states().add(
                copyLastStateWithAsset(
                    categoriesAdapter.selectedCategoryTypeResource, avatarStateAsset
                )
            )
            undoedStates().clear()
            update()
        ...
        categoriesAdapter = CategoriesAdapter { partTypeSelectorItem ->
        ...
        itemsAdapter = ItemsAdapter(requireContext()) {
            ...
            val avatarStateAsset = AvatarStateAsset(it.id,
                assetsConfig.allAssetSets().find { assetSet -> assetSet.id == it.id }?.variants?.let { variants ->
                    val selected =
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val configJson =
            File("${NMRAvatarsSDK.getResourcesDirPath(requireContext())}/${Constants.CONFIG_JSON_PATH}").inputStream()
                .use {
