package com.numplates.nomera3.presentation.view.ui.mediaViewer.viewer.adapter
...
import com.numplates.nomera3.presentation.view.widgets.VideoControlView
...
internal class ImagesPagerAdapter : RecyclingPagerAdapter<RecyclingPagerAdapter.ViewHolder>() {
    ...
    private var startVideoOnCreate: Boolean = true
    ...
    fun disableStartVideoOnCreate() {
        startVideoOnCreate = false
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int, position: Int): RecyclingPagerAdapter.ViewHolder =
            when (viewType) {
                VIEW_TYPE_VIDEO -> VideoViewHolder(parent.inflate(R.layout.item_video_viewer_pager))
                VIEW_TYPE_VIDEO_NOT_PLAYING -> getHolder(parent, position)
                else -> getHolder(parent, position)
            }

    private fun getHolder(parent: ViewGroup, position: Int): ViewHolder {
        ...
        repeatShowToolbar()
        Timber.d("ImagePageAdapter position = $position")
        pvMediaViewer.apply {
            isEnabled = true
    ...
    private fun onLongImagePress() {
        autoCloseHandler.removeCallbacks(autoCloseTask)
        toolbarAnimator?.hideToolbar()
    }

    private fun repeatShowToolbar() {
    ...
    fun getVideoViewHolderForPosition(position: Int): VideoViewHolder? {
        val holder = getViewHolderByPosition(position)
        return holder as? VideoViewHolder
    }
    ...
    inner class VideoViewHolder(val view: View) : RecyclingPagerAdapter.ViewHolder(view) {
        ...
        init {
            ...
            videoPlayer = ExoPlayer.Builder(itemView.context)
        ...
        fun bind(position: Int) {
            ...
            itemView.context?.let {
                val videoUrl = dataList[position].imageUrl ?: ""
                startVideo(videoUrl, it)
            }
        ...
        private fun startVideo(media: String, context: Context) {
            Timber.d("startVideo")
                ...
                override fun onPlayerError(error: PlaybackException) {
                    ...
                    flTest.setOnClickListener {
                        ...
                        bindPosition?.let { nonNullPosition ->
                            val videoUrl = dataList[nonNullPosition].imageUrl ?: ""
                            startVideo(videoUrl, it.context)
                        }
                    }
                }
