...
import com.numplates.nomera3.presentation.view.adapter.newfriends.MeeraOutgoingFriendsListAdapter
import com.numplates.nomera3.presentation.view.fragments.privacysettings.adapters.MeeraCellOutgoingShimmerAdapter
...
class MeeraOutgoingFriendListFragment : MeeraBaseFragment(
    ...
    companion object {
        ...
        fun newInstance(
            mode: Int, userID: Long, openedType: FriendsHostOpenedType = FriendsHostOpenedType.OTHER
        ): MeeraOutgoingFriendListFragment {
            val args = Bundle()
            args.putInt(ARG_FRIEND_LIST_MODE, mode)
            args.putLong(ARG_USER_ID, userID)
            args.putSerializable(ARG_FRIENDS_HOST_OPENED_FROM, openedType)
            val fragment = MeeraOutgoingFriendListFragment()
    ...
    private val outgoingFriendshipViewModel: OutgoingFriendshipRequestListViewModel by viewModels()

    private var mode: Int = GetFriendsListUseCase.FRIENDS
    private var userID: Long? = null

    private var adapter: MeeraOutgoingFriendsListAdapter? = null
    private val shimmerAdapter = MeeraCellOutgoingShimmerAdapter()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProviders.of(this).get(MeeraMyFriendListFragmentViewModel::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        outgoingFriendshipViewModel.getOutgoingFriendRequestList(0)
    }

    private fun initRecycler() {
        binding?.rvMyFriendsList?.layoutManager = LinearLayoutManager(context)
        adapter = MeeraOutgoingFriendsListAdapter(::initFriendClickListener)
    ...
    private fun initObservers() {
        outgoingFriendshipViewModel.foundRequests.observe(
            viewLifecycleOwner, Observer { foundRequests: MutableList<UserSimple>? ->
                foundRequests?.let {
        ...
        outgoingFriendshipViewModel.requests.observe(
            viewLifecycleOwner, Observer { outRequests: MutableList<UserSimple>? ->
                outRequests?.let {
                    if (it.size > 0) {
                        adapter?.submitList(it)
                        binding?.srMyFriendsList?.setRefreshing(false)
        ...
        when (action) {
            is FriendsListAction.OpenProfileClick -> {
                findNavController().safeNavigate(
                    resId = R.id.action_meeraOutgoingFriendListFragment_to_userInfoFragment, bundle = bundleOf(
            ...
            is FriendsListAction.CancelOutgoingFriendshipClick -> {
                outgoingFriendshipViewModel.cancelOutgoingFriendshipRequest(action.model)
            }
    ...
    private fun showPlaceHolderIfEmptyAdapter() {
        if (adapter?.itemCount == 0) {
            binding?.tvPlaceholderDescription?.text = getString(R.string.outgoing_friends_list_is_empty)
    ...
    fun search(query: String) {
        outgoingFriendshipViewModel.search(query, 0)
    }
    ...
    private fun initListeners() {
        binding?.btnSearchFriends?.setThrottledClickListener {
