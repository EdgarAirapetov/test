...
class MeeraTextViewWithImages : AppCompatTextView {
    ...
    private var ellipsizeListeners: CopyOnWriteArrayList<EllipsizeListener>? = CopyOnWriteArrayList()
...
//                    workingText = workingText.append(ELLIPSIS)
            ...
            val iterator = ellipsizeListeners?.iterator() ?: return
    ...
    fun clearAllSpans() {
        (text as? Spannable)?.getSpans(0, text.length, Any::class.java)?.forEach {
            (text as? Spannable)?.removeSpan(it)
        }
    }
    ...
    companion object {
         ...
         *
         * @see com.numplates.nomera3.presentation.view.ui.MeeraTextViewWithImages
         */
