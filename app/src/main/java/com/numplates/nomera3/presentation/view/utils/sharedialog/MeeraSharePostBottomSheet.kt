...
import com.numplates.nomera3.databinding.MeeraShareContainerFragmentBinding
...
/**
 * 24.01.25 Оставил для примера. В будущем удалить
 */
@Deprecated("Не используется т.к. высота шторки фиксированная https://nomera.atlassian.net/browse/BR-35117")
class MeeraSharePostBottomSheet(
...
) : UiKitBottomSheetDialog<MeeraShareContainerFragmentBinding>() {
    override val inflateContent: (LayoutInflater, ViewGroup?, Boolean) -> MeeraShareContainerFragmentBinding
        get() = MeeraShareContainerFragmentBinding::inflate
    ...
    private fun initPostLayout() {
        ...
        if (post.smallImage != null && post.smallImage != "") {
            contentBinding?.vPost?.setLeftUserPicConfig(
                UserpicUiModel(
                    userAvatarUrl = post.smallImage
                )
            )
        } else {
            contentBinding?.vPost?.setLeftUserPicConfig(
                UserpicUiModel(
                    userAvatarUrl = post.smallUrl
                )
            )
        }
    }
