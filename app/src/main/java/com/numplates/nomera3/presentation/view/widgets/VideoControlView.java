 ...
 */
@Deprecated()   // Use Class from CORE module
public class VideoControlView extends FrameLayout {
    ...
    public VideoControlView(
        ...
        soundButton = findViewById(R.id.exo_sound_button);
        if (soundButton != null){
    ...
    public void onSoundButtonClicked() {
        if(getPlayer() instanceof SimpleExoPlayer){
            ...
            if (volume == 0f){
                player.setVolume(1f);
                if (soundButton != null){
                    ViewKt.loadGlide(soundButton, R.drawable.ic_outlined_sound_max_m);
                }
            }else if (volume == 1f) {
                player.setVolume(0f);
                if (soundButton != null){
                    ViewKt.loadGlide(soundButton, R.drawable.ic_outlined_sound_off_m);
                }
            }
        }
    }
    ...
    private void updateProgress() {
        ...
        if (positionView != null && !scrubbing) {
            positionView.setText(Util.getStringForTime(formatBuilder, formatter,duration - position));
        }
        if (timeBar != null) {
