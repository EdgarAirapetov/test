...
class CountryHolderNew(itemView: View, private val callback: ProfileListCallback) : RecyclerView.ViewHolder(itemView), ProfileListHolder {

    internal var content: View? = itemView.findViewById(R.id.llContent)
    var tvName: TextView? = itemView.findViewById(R.id.tvName)
    var ivPicture: ImageView? = itemView.findViewById(R.id.ivPicture)

    init {
        content!!.setOnClickListener { v -> callback.onClick(this) }
    }

    override fun bind(item: ProfileListItem) {

//        imageUrl is a ResourceId !!!
        Glide.with(itemView.context)
                .load(Integer.parseInt(item.imageUrl!!))
