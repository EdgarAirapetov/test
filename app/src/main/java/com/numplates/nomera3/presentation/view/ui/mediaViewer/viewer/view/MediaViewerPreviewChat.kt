...
class MediaViewerPreviewChat @JvmOverloads constructor(
    ...
    init {
        View.inflate(context, R.layout.view_image_viewer_preview_chat, this)
    ...
    fun initUniqueNameSearch(uniqueNameSuggestionMenu: SuggestionsMenu?) {
        ...
        comment?.setDefaultTextColor(EditTextExtended.DefaultTextColor.MediaViewerPreviewChat)
    ...
    private fun handleClickCheckAction() {
        ...
        if (imagesAdapter?.getCurrentData(currentPosition)?.isSelected == false) {
            val totalSelected = getTotalSelectedMediaCount()
            if (totalSelected >= MAX_PICTURE_COUNT) {
                showHint(context.getString(R.string.you_can_add_5_objects))
                return
            }
            val selected = imagesAdapter?.getSelectedData()
            val preselected = onGetPreselectedMedia?.invoke()
            if (preselected?.any { filesUtil.isVideoUri(it) } == true
                || selected?.any { it.viewType == VIEW_TYPE_VIDEO_NOT_PLAYING } == true) {
            ...
            if (totalSelected > 0 && currentData?.imageUrl?.let { filesUtil.isVideoUri(Uri.parse(it)) } == true) {
                if (currentData.viewType == VIEW_TYPE_IMAGE) {
                    showHint(context.getString(R.string.photo_adding_not_allowed))
                } else {
                    showHint(context.getString(R.string.video_adding_not_allowed))
            ...
            val strUriToCheck = currentData?.imageUrl?.let {
                Uri.parse(it)
            }

            val needEditResponse = act?.getMediaControllerFeature()?.needEditMedia(
                uri = strUriToCheck,
                openPlace = MediaControllerOpenPlace.Chat
            )

            if (needEditResponse is MediaControllerNeedEditResponse.VideoTooLong) {
                act?.getMediaControllerFeature()?.showVideoTooLongDialog(
                    openPlace = MediaControllerOpenPlace.Chat,
                    needEditResponse = needEditResponse,
                    showInMinutes = true
