...
class MeeraSignalingService : LifecycleService(), AppRTCClient {

    var onStartCall: ((Boolean, UserChat?, Boolean?) -> Unit)? = null

    private val binder: IBinder = MeeraSignalingServiceBinder()
    ...
    override fun onBind(intent: Intent): IBinder {
        (binder as? MeeraSignalingServiceBinder)?.setService(this)
        Timber.v("ON_BIND SignalingService")
        return binder
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_INCOMING_CALL) {
            val pushCallObject = gson.fromJson(intent.getStringExtra(IArgContainer.ARG_PUSH_OBJECT), PushCallObject::class.java)
            ...
            callUser = intent.getParcelableExtra(IArgContainer.ARG_USER_MODEL) as? UserChat
            isIncoming.set(true)
            videoCall = false
    ...
    private fun handleUsersIncomingCall(intent: Intent?, typeCallAction: Int) {
        ...
        val pushCallUser = gson.fromJson(intent?.getStringExtra(IArgContainer.ARG_PUSH_OBJECT), PushCallObject::class.java)
        callNotificationManager.cancelCallNotification()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
    ...
    override fun onCreate() {
        ...
        super.onCreate()
        Timber.d("onCreate => SignallingService")
    ...
    override fun onUnbind(intent: Intent?): Boolean {
        (binder as? MeeraSignalingServiceBinder)?.removeService()
        stopSelf()
    ...
    fun observeSignalingChannel(block: () -> Unit) {
        Timber.e("MEERA_SERVICE_LOG OBSERVE SignaLLING SERVICE >>>>>>>>>>>>>>>>>> ${disposable?.isDisposed}")
        if (disposable != null && !disposable!!.isDisposed) {
            disposable?.dispose()
            disposable = null
        }
        disposable = webSocketChannel.observeSignallingChannel()
            ...
            .subscribe({ response ->
                ...
                Timber.d("MEERA_SERVICE_LOG SIGNALLING_SERVICE_OBSERVER RESPONSE >>>>>>>>>>>>> TYPE(${fromRemote.type}) RESPONSE:${response.payload}")
    ...
    private fun inOutgoingCallState(evt: SignalingEvent) {
        ...
        if (evt is SignalingIncomingCall) {
            ...
            Timber.e(" MEERA_CALL_LOG evt is SignalingAcceptCall")
            callState.set(SignalingStates.SIGNALING_WEBRTC_SESSION)
            callProgressNotification?.get()?.callIsAccepted(roomIdInProc, uuidInProc)
    ...
    private fun entryIncomingCallState() {
        Timber.d("[entryIncomingCallState]")
        ...
        if (!isIncoming) {
            ...
            if (roomId != null && messageId != null) {
                val remoteObject = SignalingMsgPayload(
                    messageId,
                    roomId,
                    DialogEntity(companion = callUser),
                    "voip",
                    AUDIO_CALL_KEY
                )
...
class MeeraSignalingServiceBinder : Binder() {
    private var service: WeakReference<MeeraSignalingService>? = null

    fun getService() = service

    fun setService(signalingService: MeeraSignalingService) {
