...
class MeeraFeedViewModel @Inject constructor(
...
//    private val readOnBoardingUseCase: ReadOnboardingUseCase,
    ...
    private var savedScrollOffset: Pair<Int, Int> = Pair(0, 0)

    private val _liveEvent = MutableLiveData<FeedViewEvent?>()
    private val _editPostEvent = SingleLiveEvent<FeedViewEvent>()
    private val _livePosts = MutableLiveData<MutableList<PostUIEntity>>()

    //todo ROAD FIX
    var editPostEvent = _editPostEvent as LiveData<FeedViewEvent>
    ...
    fun showReactionStatistics(
        ...
        _liveEvent.postValue(FeedViewEvent.ShowReactionStatisticsEvent(post, entityType))
    }
    ...
    fun onTriggerAction(action: FeedViewActions) {
        if (roadType == null && action != FeedViewActions.CheckIfInitialOpen) return
        ...
        when (action) {
            ...
            is FeedViewActions.SubscribeToPost -> subscribeToPost(action.postId)
            is FeedViewActions.UnsubscribeFromPost -> unsubscribeFromPost(action.postId)
            ...
            is FeedViewActions.CheckIfInitialOpen -> handleCheckingIfInitialOpen()
    ...
    fun downloadPostVideo(postId: Long, assetId: String?) {
        downloadVideoToGalleryUseCase.invoke(
            DownloadMediaHelper.PostMediaDownloadType.PostRoadDownload(
                postId,
                assetId
            )
        )
    ...
    // TODO FIX with onboarding
//    fun isNeedToShowOnBoarding() = false /*readOnBoardingUseCase.invoke()*/
    ...
    fun getSavedScrollOffset(): Pair<Int,Int> = savedScrollOffset

    fun savePositionAndScrollOffset(position: Int, offset: Int) {
        savedScrollOffset = Pair(position, offset)
    }
    ...
    private fun handleCheckingIfInitialOpen() {
        val posts = _livePosts.value ?: arrayListOf()
        if (posts.isEmpty()) _liveEvent.value = FeedViewEvent.LoadInitialPosts
    }
    ...
    private fun updatePostSelectedMediaPosition(postId: Long, selectedMediaPosition: Int) = runCatching {
        val postIndex =
            _livePosts.value?.indexOfFirst { it.postId == postId }.takeIf { (it ?: 0) > -1 } ?: return@runCatching
        val updatedPost = _livePosts.value?.get(postIndex)?.copy(selectedMediaPosition = selectedMediaPosition)
            ?: return@runCatching
    ...
    private fun handleSyncWork(workInfo: WorkInfo?) {
        ...
        if (isFinishedSucceed) {
            if (getSyncContactsPrivacyUseCase.invoke()
                    .not() && _liveEvent.value !is FeedViewEvent.ShowContactsHasBeenSyncDialog
            ) {
    ...
    private fun loadSuggests() {
        viewModelScope.launch {
            runCatching {
                ...
                this@MeeraFeedViewModel.suggests.clear()
                this@MeeraFeedViewModel.suggests.addAll(suggestsUiModels)
...
//        }
        ...
        val shimmerItems = listOfNotNull(momentShimmer, postShimmer)
    ...
    private fun updateUserMomentsState(userMomentsStateUpdate: UserMomentsStateUpdateModel) {
        ...
        liveFeedEvents.value =
            FeedViewEventPost.UpdatePostEvent(postUpdate.copy(postMomentsBlock = momentItem))
    }

    private fun getUpdatedMomentsItem(
        postUpdate: UIPostUpdate.UpdateUserMomentsState,
        post: PostUIEntity
    ): PostUIEntity {
    ...
    private fun updateMomentsGroupItem(
        postUpdate: UIPostUpdate.UpdateUserMomentsState,
        momentsGroup: MomentGroupUiModel
    ): MomentGroupUiModel? {
            ...
            try {
                ...
                if (result.data != null) {
                    _liveEvent.postValue(
                        FeedViewEvent.OnSuccessHideUserRoad(
                            userId,
                            R.string.disabled_new_post_notif
                        )
                    )
                    val newList = getActivePosts().filter { it.user?.userId != userId }
                    handlePlaceHolders(newList.size)
                    postId?.let { postId ->
    ...
    private fun handleSuccessSubscribePost() {
        _liveEvent.postValue(FeedViewEvent.ShowCommonSuccess(R.string.subscribe_post))
    }

    private fun unsubscribeFromPost(postId: Long?) {
        viewModelScope.launch(Dispatchers.IO) {
            postId?.let { id ->
                val params = UnsubscribePostParams(id)
                unsubscribePostUseCase.execute(params, {
                    handleSuccessUnSubscribePost()
                }, { exception ->
    ...
    private fun handleSuccessUnSubscribePost() {
        _liveEvent.postValue(FeedViewEvent.ShowCommonSuccess(R.string.unsubscribe_post))
    }
    ...
    private fun getPostsCommon(
        ...
        loadingPostsJob = viewModelScope.launch(Dispatchers.IO) {
            ...
            getPosts.execute(params, { response ->
                ...
                val type = this@MeeraFeedViewModel.roadType
                launch(Dispatchers.Main) {
                    if (startPostId == 0L) {
                        ...
                        _liveEvent.value = FeedViewEvent.OnFirstPageLoaded(posts, features)
                    } else {
                        appendListAndSubmit(
                            posts = posts,
                            momentsBlockAvatar = momentsBlockAvatar,
                    ...
                    _liveEvent.postValue(FeedViewEvent.TotalPostCount(response.hashtag?.count ?: 0))
    ...
    private fun addPostsViewedBlockProfile(
        ...
        if (!isUserAuthorizedUseCase.invoke()) return
    ...
    private fun submitList(data: List<PostUIEntity>) {
        viewModelScope.launch(Dispatchers.Main) {
    ...
    private fun generateLocalFeatures(momentsBlockAvatar: String?): List<PostUIEntity> {
        val moments = LocalPosts.getMeeraMoments(
...
//            roadType = roadType
        ...
        return listOfNotNull(
            ...
            LocalPosts.getSuggestsPost(suggests).takeIf {
                roadType == RoadTypesEnum.MAIN &&
                    getUserUid() != DEFAULT_ANON_UID
            },
            LocalPosts.getSyncContactsPost().takeIf {
                roadType == RoadTypesEnum.MAIN &&
                    getUserUid() != DEFAULT_ANON_UID &&
    ...
    private fun observeEventParticipationChanges() {
        viewModelScope.launch {
            observeEventParticipationChangesUseCase.invoke()
                ...
                .catch {
                    Timber.e(it)
                    _liveEvent.postValue(FeedViewEvent.ShowCommonError(R.string.general_toast_error))
                }
                .launchIn(viewModelScope)
        }
    }
    ...
    private fun observeFeedState() {
        getFeedStateUseCase.execute(DefParams()).subscribeOn(Schedulers.io()).observeOn(
    ...
    private fun handleFeedUpdate(updatedItem: FeedUpdateEvent?) {
        updatedItem?.let { item ->
            when (item) {
                ...
                is FeedUpdateEvent.FeedUpdateAll -> subjectPosts.onNext(
                    FeedViewEvent.UpdatePostById(
                        item.postId
                    )
                )
    ...
    private fun handleFeedHideUserRoad(item: FeedUpdateEvent.FeedHideUserRoad) {
        if (roadType == RoadTypesEnum.MAIN || roadType == RoadTypesEnum.COMMUNITY || roadType == RoadTypesEnum.HASHTAG) {
