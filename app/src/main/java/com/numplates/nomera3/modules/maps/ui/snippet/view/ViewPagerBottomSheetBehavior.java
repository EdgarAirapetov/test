 ...
 * a bottom sheet.
 * @deprecated Deprecated in app module. Use UiKitViewPagerBottomSheetBehavior
 */
@Deprecated
public class ViewPagerBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    private static final float HORIZONTAL_DRAG_BIAS_FACTOR = 1.5f;
    ...
    CustomViewDragHelper mViewDragHelper;
    ...
    WeakReference<View> mNestedScrollingChildRef = new WeakReference<>(null);

    private final List<BottomSheetCallback> callbacks = new ArrayList<>();
    ...
    private boolean mNestedPageScrollingEnabled = true;

    private boolean mExpandedStateRestricted = false;
    ...
    public boolean onLayoutChild(@NotNull CoordinatorLayout parent, @NotNull V child, int layoutDirection) {
        ...
        if (mViewDragHelper == null) {
            ...
            mViewDragHelper.setDraggingUpRestricted(mExpandedStateRestricted);
        }
    ...
    public boolean onInterceptTouchEvent(@NotNull CoordinatorLayout parent, V child, @NotNull MotionEvent event) {
        if (mInterceptTouchEvents) return true;
        if (!child.isShown()) {
            mIgnoreEvents = true;
            return false;
        }
        ...
        float dx = Math.abs(mInitialX - event.getX());
        float dy = Math.abs(mInitialY - event.getY());
        boolean isVerticalDrag = dx * HORIZONTAL_DRAG_BIAS_FACTOR <= dy;

        if (!mIgnoreEvents && (isVerticalDrag || action == MotionEvent.ACTION_DOWN) && mViewDragHelper.shouldInterceptTouchEvent(event)) {
            return true;
        }
        ...
        View scroll = mNestedScrollingChildRef.get();
        return !mNestedPageScrollingEnabled && action == MotionEvent.ACTION_MOVE && scroll != null &&
            !mIgnoreEvents && mState != STATE_DRAGGING &&
            !parent.isPointInChildBounds(scroll, (int) event.getX(), (int) event.getY()) &&
            (dy > mViewDragHelper.getTouchSlop() && isVerticalDrag);
    }
    ...
    public boolean onTouchEvent(@NotNull CoordinatorLayout parent, V child, @NotNull MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_MOVE && !mIgnoreEvents) {
            if (dy > mViewDragHelper.getTouchSlop() && isVerticalDrag) {
    ...
    public void invalidateScrollingChild(boolean nestedPageScrollingEnabled) {
        if (mViewRef==null) {
            return;
        }
        mNestedPageScrollingEnabled = nestedPageScrollingEnabled;
        if (nestedPageScrollingEnabled) {
            final View scrollingChild = findScrollingChild(mViewRef.get());
            mNestedScrollingChildRef = new WeakReference<>(scrollingChild);
        } else {
            mNestedScrollingChildRef = new WeakReference<>(null);
        }
    }

    public void setExpandedStateRestricted(boolean restricted) {
        mExpandedStateRestricted = restricted;
        if (mViewDragHelper != null) {
            mViewDragHelper.setDraggingUpRestricted(restricted);
        }
    }
    ...
    public final void setState(final @State int state) {
        ...
        if (state == STATE_EXPANDED && mExpandedStateRestricted) return;
        if (mViewRef == null) {
    ...
    void setStateInternal(@State int state) {
        ...
        mState = state;
        mInterceptTouchEvents = state == STATE_SETTLING;
    ...
    View findScrollingChild(View view) {
        if (!mNestedPageScrollingEnabled) return null;
    ...
    void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !callbacks.isEmpty()) {
            if (top > mMaxOffset) {
                for (int i = 0; i < callbacks.size(); i++) {
                    callbacks.get(i).onSlide(bottomSheet, (float) (mMaxOffset - top) /
