...
class MeeraPeopleContentUiMapper @Inject constructor(
    ...
    fun createDefaultContent(
        ...
        when {
            !lastRecentItems.isNullOrEmpty() && showLastRecentItems -> {
                listResult.add(createRecentPeopleUiEntity(lastRecentItems))
            }
            showLastRecentItems -> {
                listResult.add(RecentUsersShimmerUiEntity)
            }
        }
        if (!showPeoples) return listResult
        listResult.add(RecommendedUsersShimmerUiEntity)
    ...
    fun createPeopleContent(
        ...
        if (peopleRelatedUserModels.isNotEmpty()) {
            val mappedRelatedUsers = mapRelatedUsers(peopleRelatedUserModels, true)
        ...
        listResult.add(
            createHeaderEntity(
                ...
                drawableTextRes = if (createForSearch) null else R.drawable.ic_outlined_info_circle_s
            )
        )
        ...
        if (!createForSearch) {
            listResult.add(
                FindPeoplesUiEntity(
                    ...
                    icon = R.drawable.ic_outlined_search_m,
        ...
        if (featureTogglesContainer.shakeFeatureToggle.isEnabled) {
            listResult.add(
                FindPeoplesUiEntity(
                    ...
                    icon = R.drawable.ic_outlined_bump_m,
