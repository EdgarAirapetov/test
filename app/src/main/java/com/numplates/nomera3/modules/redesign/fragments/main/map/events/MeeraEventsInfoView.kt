...
import com.numplates.nomera3.databinding.MeeraViewEventsInfoBinding
...
private const val ONBOARDING_EVENT_ANIMATION_DURATION = 2000L

class MeeraEventsInfoView @JvmOverloads constructor(
    context: Context,
    ...
    private val binding = LayoutInflater.from(context)
        .inflate(R.layout.meera_view_events_info, this, false)
        .apply(::addView)
        .let(MeeraViewEventsInfoBinding::bind)
    ...
    init {
        ...
        binding.tvMapEventsInfoRules.text = context.getString(R.string.map_events_info_bullet_3).addClickableText(
            ContextCompat.getColor(context, R.color.uiKitColorAccentPrimary),
        ...
        binding.laEventAnimation.playAnimation()

        binding.laEventAnimation.addAnimatorListener(object : AnimatorListener {
            override fun onAnimationStart(p0: Animator) {
                p0.doOnResume {
                    postDelayed({
                        binding.laEventAnimation.pauseAnimation()
                    }, ONBOARDING_EVENT_ANIMATION_DURATION)
                }
            }
