...
import com.numplates.nomera3.databinding.ItemAddPostAttachmentNewBinding
...
//TODO: remove after redisgn
interface BasePostAttachmentView {
    fun getImageYPosition(): Double
    fun updateMaxHeight(mediaPreviewMaxHeight: Int)
}

@SuppressLint("ClickableViewAccessibility")
class MeeraAddPostAttachmentItemViewNew @JvmOverloads constructor(
    context: Context,
    ...
    private val binding = LayoutInflater.from(context)
        .inflate(R.layout.item_add_post_attachment_new, this, false)
        .apply(::addView)
        .let(ItemAddPostAttachmentNewBinding::bind)
    ...
    /**
     * CornerRadius добавлен чтобы можно было выпрямлять углы во вью для ивентов
     * */
    private var imageTopCornersRadius:Int = 16
    ...
    fun bind(
        ...
        cornerRadius: Int = imageTopCornersRadius
    ) {
        ...
        imageTopCornersRadius = cornerRadius
    ...
    private fun setupViews(attachment: UIAttachmentPostModel) {
        ...
        with(binding) {
            ivMediaAttachment.bind(onImageSet = {
        ...
        with(binding) {
            ivMediaAttachment.apply {
                ...
                val requestOptions =  RequestOptions()
                    .transform(
                        CenterCrop(),
                        GranularRoundedCorners(imageTopCornersRadius.dp.toFloat(), imageTopCornersRadius.dp.toFloat(), 0f, 0f)
                    )
                loadGlideWithOptions(attachment.attachmentResource, requestOptions)
            }
        }
    }
