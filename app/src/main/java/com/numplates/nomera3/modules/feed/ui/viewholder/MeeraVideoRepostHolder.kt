...
class MeeraVideoRepostHolder(
    ...
    private var zoomChangeListener: ZoomListener? = null
    private var zoomTapListener: TapListener? = null
    private var canPerformZoomListener: CanPerformZoom? = null
    private var videoToggleVolumeClickListener: OnClickListener? = null
    ...
    override fun setupContent(post: PostUIEntity) {
        ...
        if (post.isParentPostDeleted()) {
            ...
            ivPicture?.invisible()
            setupImageAspect(aspect, parentFinalWidth - SUM_LEFT_RIGHT_MARGIN)
    ...
    override fun setupZoom(post: PostUIEntity) {
        runCatching {
            itemView.post {
                ...
                zoomChangeListener = object : ZoomListener {
    ...
    override fun setupBlur(post: PostUIEntity) {
        ...
        contentManager?.let { cm ->
            if ((post.parentPost?.isAdultContent == true
                ...
                && !cm.isMarkedAsNonSensitivePost(post.parentPost.postId)
            ) {
    ...
    override fun clearResources() {
        ...
        flMediaContainer?.setOnClickListener(null)
        cacheUtilTool = null
        detachPlayer()
    ...
    override fun needToPlay(): Boolean {
        ...
        return !((post?.parentPost?.isAdultContent == true
            && !image.isNullOrEmpty()) && (contentManager?.isMarkedAsNonSensitivePost(post?.parentPost?.postId) != true))
    }
    ...
    private fun initVideoPlayer(onFinished: (() -> Unit)? = null) {
        val media = getVideoUrlString() ?: return
    ...
    private fun setupVideoDurationLabel(post: PostUIEntity) {
        videoDurationView = itemView.findViewById(R.id.vdv_parent_post_video_duration)
    ...
    private fun showVideoDurationIfNeeded(post: PostUIEntity, isNotPlaying: Boolean = true) {
        videoDurationView?.isVisible =
            isNotPlaying && !isNeedShowBlur(post.parentPost ?: post) && !post.isParentPostDeleted()
    }
    ...
    private fun updateVideoDurationView(isPlaying: Boolean) {
        ...
        if (duration != 0L) {
            videoDurationView?.setDurationText(
                getDurationSeconds(
    ...
    private fun removeVideoControlView() {
        videoDurationControl?.let {
            it.player = null
            flMediaContainer?.removeView(it)
    ...
    private fun initVideoDurationControlView() {
        ...
        videoToggleVolumeClickListener = OnClickListener { toggleVolume() }
        videoDurationControl =
            CustomControlView(itemView.context, R.layout.feed_exo_player_control_view_extended).also { view ->
                view.apply {
                    setPadding(videoDurationPaddingPx)
                    disableTimeRound()
                    setOnClickListener(videoToggleVolumeClickListener)
                    ...
                    post?.let {
                        val isNotPlaying = videoView?.player?.isPlaying != true
                        showVideoDurationIfNeeded(post = it, isNotPlaying = isNotPlaying)
                    } ?: run { videoDurationView?.visible() }
                }
            }
    }
