...
class MomentSettingsNotShowSearchMomentUseCase @Inject constructor(private val repository: MomentsRepository) {
    suspend fun invoke(
        name: String,
        limit: Int,
        offset: Int
    ): ResponseWrapper<UsersWrapper<UserSimple>> {
        return repository.searchNotShowExclusion(name, limit, offset)
    }
}
