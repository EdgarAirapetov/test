...
import com.numplates.nomera3.databinding.ItemPostMultimediaItemBinding
...
class PostMultimediaPagerItemView @JvmOverloads constructor(
    context: Context,
    ...
    private val binding = LayoutInflater.from(context)
        .inflate(R.layout.item_post_multimedia_item, this, false)
        .apply(::addView)
        .let(ItemPostMultimediaItemBinding::bind)
    ...
    private var canZoom: Boolean = true
    private var contentManager: ISensitiveContentManager? = null
    private var onItemClicked: (MediaAssetEntity, PostUIEntity?) -> Unit = { _, _ ->}
    ...
    fun stopPlayingVideo() {
        ...
        binding.vdvPostMultimediaVideoDuration.visible()
    }
    ...
    private fun initVideoPlayer() {
        val media = this@PostMultimediaPagerItemView.media?.video ?: return
        val ctx = context?.applicationContext ?: return

        videoPlayer = VideoUtil.getFeedHolderPlayer(ctx).also { it.repeatMode = Player.REPEAT_MODE_ALL }

        binding.pvPostMultimediaItemVideoView.player = videoPlayer

        val mediaSource = VideoUtil.getFeedHolderPlayerMediaSource(ctx, media)
    ...
    private fun initVideoDurationControlView() {
        ...
        videoDurationControl = CustomControlView(context, R.layout.feed_exo_player_control_view_extended).also { view ->
            view.apply {
    ...
    private fun removeVideoControlView() {
        videoDurationControl?.let { binding.flPostMultimediaItemMediaContainer.removeView(it) }.also { videoDurationControl = null }
    }
    ...
    private fun updateVideoDurationView(isPlaying: Boolean) {
        ...
        if (duration != 0L) {
            binding.vdvPostMultimediaVideoDuration
                .setDurationText(getDurationSeconds((duration - currentPositionInSeconds).fromMillisToSec().toInt()))
        }
