...
class FeatureHolder(
    ...
    private val tvTextPost = itemView.findViewById<TextViewWithImages>(R.id.tv_text_post)
    private val btnAction = itemView.findViewById<TextView>(R.id.tv_btn_action)
    private val btnDismiss = itemView.findViewById<TextView>(R.id.tv_btn_dismiss)
    ...
    override fun bind(post: PostUIEntity) {
        ...
        setupContent(post)
        setVolume(volumeStateCallback.getVolumeState())
    }
    ...
    fun setVolume(volumeState: VolumeState){
        ...
        videoDurationView.click {
            val newVolume = when (volumeStateCallback.getVolumeState()) {
    ...
    private fun setupClickListeners(featureData: FeatureData) {
        ...
        ivCloseAnnounce?.setOnClickListener {
            postCallback?.onFeatureClicked(
                featureId = featureData.id,
                haveAction = haveActionButton,
                dismiss = true,
