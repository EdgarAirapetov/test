...
import com.numplates.nomera3.modules.gifservice.ui.MeeraMediaKeyboard
...
class MeeraEditorWidgetsFragment : MeeraBaseFragment(R.layout.editor_widgets_fragment),
    ...
    private val binding by viewBinding(EditorWidgetsFragmentBinding::bind)

    private val viewModel by viewModels<EditorWidgetsViewModel> { App.component.getViewModelFactory() }

    private var mediaKeyboard: MeeraMediaKeyboard? = null
    ...
    private fun initMediaKeyboard() {
        val bottomSheetBehavior = BottomSheetBehavior
            .from<View>(binding.bottomSheetRecycler)
            ...
            object : BottomSheetBehavior.BottomSheetCallback() {
                override fun onStateChanged(bottomSheet: View, newState: Int) {
                    if (newState == BottomSheetBehavior.STATE_HIDDEN) {
                        val params = requireActivity().window.attributes.apply { alpha = 0f }
    ...
    override fun onWidgetClicked(widget: MediaKeyboardWidget) {
        when (widget) {
            MediaKeyboardWidget.MUSIC_WIDGET -> (requireActivity() as MomentsWrapperActivity).openAudioPlayer()
