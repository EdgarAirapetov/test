...
class MeeraMapFriendsListRecyclerView @JvmOverloads constructor(
    context: Context,
    ...
    private var stateBottomSheet: Int = STATE_HALF_EXPANDED
    ...
    fun init(
        ...
        val eventItemSelectionListener = object : OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                val item = (adapter as MeeraEventFriendsListItemAdapter).items?.getOrNull(itemPosition)
        ...
        adapter = MeeraEventFriendsListItemAdapter { itemActionListener?.invoke(it) }
    ...
    override fun onInterceptTouchEvent(e: MotionEvent): Boolean {
        if (stateBottomSheet == STATE_HALF_EXPANDED) {
    ...
    fun setItems(items: List<MapFriendListItem>) {
        (adapter as MeeraEventFriendsListItemAdapter).items = items
    }

    fun getFirstItem() =
        ((adapter as? MeeraEventFriendsListItemAdapter)?.items?.firstOrNull() as? MapFriendListItem.MapFriendUiModel)

    fun updateStateBottomSheet(state: Int) {
