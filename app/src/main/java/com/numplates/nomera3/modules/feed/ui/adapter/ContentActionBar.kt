...
import com.numplates.nomera3.databinding.PostActionBarCustomBinding
...
import com.numplates.nomera3.modules.reaction.ui.custom.ReactionBadge
import com.numplates.nomera3.presentation.view.ui.mediaViewer.common.extensions.globalVisibleRect


private const val BUBBLE_VIEW_HEIGHT = 70
private const val BUBBLE_VIEW_BOTTOM_MARGIN = 5
...
class ContentActionBar @JvmOverloads constructor(
    context: Context,
    ...
    private val binding: PostActionBarCustomBinding =
        PostActionBarCustomBinding.inflate(LayoutInflater.from(context), this, true)

    init {
        binding.rbReactions.setPostType()
    }

    fun getReactionBadge(): ReactionBadge {
    ...
    private fun updateActionBar(params: Params, contentActionBarType: ContentActionBarType) {
        binding.rlbPostActionBarLikeButton.setButtonEnabledAlpha(params.isEnabled)
        if (params.isMoment) {
    ...
    private fun initListeners(contentActionBarType: ContentActionBarType) {
        ...
        if (params?.isEnabled == true) {
            binding.rlbPostActionBarLikeButton.onClick(contentActionBarType) {
        ...
        binding.rbReactions.setListener { reactionBadgeEvent ->
            if (reactionBadgeEvent is ReactionBadge.Event.Tap || reactionBadgeEvent is ReactionBadge.Event.LongClick) {
    ...
    private fun createPointForReactionBubble(): Point {
        binding.rlbPostActionBarLikeButton.globalVisibleRect.let { rect ->
            ...
            val topForReactionBubble = rect.top - dpToPx(BUBBLE_VIEW_HEIGHT)
            return if (topForReactionBubble - dpToPx(BUBBLE_VIEW_HEIGHT) <= 0) {
        ...
        companion object {
            ...
            fun getType(params: Params?): ContentActionBarType {
                return when {
                    ...
                    params?.userAccountType == AccountTypeEnum.ACCOUNT_TYPE_VIP.value -> DARK
                    else -> DEFAULT
                }
            }
        }
