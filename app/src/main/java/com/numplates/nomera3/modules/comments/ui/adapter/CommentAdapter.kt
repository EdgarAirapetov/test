...
import com.numplates.nomera3.modules.comments.ui.viewholder.CommentProgressViewHolder
import com.numplates.nomera3.modules.comments.ui.viewholder.CommentSeparatorViewHolder
import com.numplates.nomera3.modules.comments.ui.viewholder.CommentViewHolder
import com.numplates.nomera3.modules.comments.ui.viewholder.CommentViewHolderPlayAnimation
import com.numplates.nomera3.modules.comments.ui.viewholder.DeletedCommentViewHolder
...
class CommentAdapter(
    ...
    private val collection: ObservableMutableList<CommentUIType> = ObservableMutableList(_collection)
    ...
    fun getItem(position: Int): CommentEntity? {
        return try {
            ...
            Timber.e("Item isn't CommentEntity. ${e.message}")
            null
        }
    }
    ...
    private fun hideAllAnswers(parentId: Long) {
        ...
        spoiler ?: return
        if (spoiler.count == 0) {
            if (newSpoilerCount > 3) spoiler.count =
                newSpoilerCount - 3 // так как при скрытии отображается макс 3 элемента
        } else if ((newSpoilerCount - 3) > spoiler.count ?: 0) { //todo заменить на пересчет с бэка
            spoiler.count = newSpoilerCount - 3
        }
    ...
    fun playCommentAnimation(commentId: Long, animation: CommentViewHolderPlayAnimation) {
        ...
        when (animation) {
            CommentViewHolderPlayAnimation.PlayLikeOnDoubleClickAnimation -> {
                notifyItemChanged(index, CommentViewHolderPlayAnimation.PlayLikeOnDoubleClickAnimation)
            }
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ITEM_TYPE_PARENT_COMMENT ->
                CommentViewHolder(
                    view = parent.inflate(R.layout.meera_item_comment),
                    callback = commentListCallback
                )

            ITEM_TYPE_ACTION_COMMENT ->
                CommentSeparatorViewHolder(parent)

            ITEM_TYPE_PROGRESS_COMMENT ->
                CommentProgressViewHolder(parent)

            ITEM_TYPE_DELETED_COMMENT ->
                DeletedCommentViewHolder(parent.inflate(R.layout.item_comment_deleted_by_post_author))
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is CommentViewHolder -> holder.bind(collection[position] as CommentEntity)

            is CommentSeparatorViewHolder ->
                holder.bind(collection[position] as CommentSeparatorEntity, insideListener)

            is CommentProgressViewHolder -> holder.bind()

            is DeletedCommentViewHolder ->
