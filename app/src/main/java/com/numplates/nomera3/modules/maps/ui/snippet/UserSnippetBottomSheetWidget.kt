...
class UserSnippetBottomSheetWidget @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyle: Int = 0
) : FrameLayout(context, attrs, defStyle), IOnBackPressed {

    private var viewModel: UserSnippetViewModel? = null
                ...
                msvpSnippetItems.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
                    ...
                    override fun onPageSelected(position: Int) {
                        lastSelectedPosition?.let {
                            ...
                            val userInfoFragment = itemEntry?.fragment as? UserInfoFragment
    ...
    fun initialize(hostFragment: Fragment, viewModel: UserSnippetViewModel) {
        ...
        viewModel.eventStateFlow
            .onEach(this@UserSnippetBottomSheetWidget::handleEvent)
    ...
    fun setUser(selectedUser: MapUserUiModel, isAuxSnippet: Boolean, fullSnippet: Boolean = false, snippet: UserSnippetModel? = null) {
        if (isAuxSnippet) {
            viewModel?.setAuxUser(selectedUser)
        } else {
            viewModel?.setSelectedUser(selectedUser, fullSnippet, snippet)
        }
        visible()
    ...
    inner class UserFragmentsPagerAdapter(fragmentManager: FragmentManager) :
        ...
        override fun getItem(position: Int): Fragment {
            val fragment = when (val item = getItemModel(position)) {
                ...
                is UserPreviewItem -> UserInfoFragment().apply {
                    arguments = Bundle().apply {
                        putLong(ARG_USER_ID, item.uid)
                        putString(ARG_TRANSIT_FROM, AmplitudePropertyWhere.USER_SNIPPET.property)
                        when (item.payload) {
                            is UserSnippetModel -> putParcelable(UserInfoFragment.ARG_USER_SNIPPET_DATA, item.payload)
                            is MapUserUiModel -> putParcelable(UserInfoFragment.ARG_USER_PIN_DATA, item.payload)
                        }
                        putBoolean(
                            UserInfoFragment.ARG_USER_SNIPPET_FOCUSED,
                            position == binding?.msvpSnippetItems?.currentItem
                        )
                    }
                    userSnippet = this@UserSnippetBottomSheetWidget
                    addListener(this)
                }
            ...
            fragment.lifecycle.addObserver(object : DefaultLifecycleObserver {
                ...
                override fun onStart(owner: LifecycleOwner) {
                    super.onStart(owner)
                    if (!isTopFragmentViewMedia(owner) && !fragment.isFragmentStarted) {
        ...
        override fun destroyItem(container: ViewGroup, position: Int, obj: Any) {
            (itemEntryMap[position]?.fragment as? UserInfoFragment)?.apply {
                userSnippet = null
                removeListener(this)
            }
            ...
            post {
                ...
                items.forEachIndexed { index, userSnippetItem ->
                    ((userSnippetItem as? UserPreviewItem)?.payload as? UserSnippetModel)?.let { userSnippetModel ->
                        (itemEntryMap[index]?.fragment as? UserInfoFragment)?.setUserSnippetModel(userSnippetModel)
                    }
                }
            }
        }
        ...
        private fun isTopFragmentViewMedia(owner: LifecycleOwner): Boolean {
            val userInfoFragment = owner as? UserInfoFragment?
            val act = userInfoFragment?.context as? Act?
            return act?.isTopFragmentViewMedia().isTrue()
        }
    }
