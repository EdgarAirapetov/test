...
// TODO: Удалить после редизайна - https://nomera.atlassian.net/browse/BR-30857
class ReactionBubbleViewController {

    private var actionHandler: ((ActActions) -> Unit)? = null

    var onReactionBubbleShow: () -> Unit? = {}
    var onReactionBubbleHide: () -> Unit? = {}
    private var currentReactionBubble: ReactionBubble? = null
    ...
    fun showReactionBubble(
        ...
        activity.needAuth {
            ...
            onBubbleShow(viewsToHide = viewsToHide, reactionTip = reactionTip)
            currentReactionBubble = ReactionBubble.show(
                position = showPoint,
...
fun Act.getDefaultReactionContainer(): ReactionBubbleViewController.ContainerInfo {
    val interceptLayout = findViewById<InterceptTouchFrameLayout>(R.id.meera_root_layout_activity)
    return ReactionBubbleViewController.ContainerInfo(
