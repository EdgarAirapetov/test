...
class MomentSettingsNotShowGetExclusionUseCase @Inject constructor(private val repository: MomentsRepository) {
    suspend fun invoke(
        limit: Int,
        offset: Int
    ): ResponseWrapper<UserWrapperWithCounter<UserSimple>> {
        return repository.getMomentsNotShowExclusions(limit, offset)
    }
}
