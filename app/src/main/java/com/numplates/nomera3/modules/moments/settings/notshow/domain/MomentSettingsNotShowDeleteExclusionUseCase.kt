...
class MomentSettingsNotShowDeleteExclusionUseCase @Inject constructor(private val repository: MomentsRepository) {
    suspend fun invoke(userIds: List<Long>): ResponseWrapper<Any> {
        val body = hashMapOf(USER_IDS_KEY to userIds)
        return repository.deleteMomentNotShowExclusion(body)
    }
}
