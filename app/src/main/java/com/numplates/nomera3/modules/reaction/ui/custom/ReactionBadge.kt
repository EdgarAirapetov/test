...
import com.numplates.nomera3.databinding.ReactionBadgeBinding
...
class ReactionBadge @JvmOverloads constructor(
    context: Context,
    ...
    private val binding: ReactionBadgeBinding =
        ReactionBadgeBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    private var viewManager = ReactionBadgeViewManager(binding.llReactionBadgeContainer)
    ...
    init {
        binding.ivAddReactionButton.setOnClickListener {
            listener?.invoke(Event.Tap)
        }
        binding.llReactionBadge.setOnClickListener {
            listener?.invoke(Event.Tap)
        }
    ...
    fun setReactions(reactions: List<ReactionEntity>, isLightText: Boolean) {
        ...
        viewManager.initReactions(this.reactions)
    ...
    private fun initCounterColor(counterView: TextView, isLightText: Boolean) {
        if (isLightText) {
            ...
            counterView.textColor(R.color.ui_gray_80)
        }
    }
