...
class UserProfileDtoDomainMapper @Inject constructor(
    private val userSimpleMapper: UserSimpleDomainMapper,
    private val featureTogglesContainer: FeatureTogglesContainer
) {
    fun mapDataToDomain(profile: UserProfileDto, price: String): UserProfileModel {
        return UserProfileModel(
            ...
            closedProfile = profile.closedProfile,
            ...
            coordinates = LocationModel(
                ...
                longitude = profile.coordinates?.lon,
                cityName = profile.city?.name ?: "",
                countryName = profile.country?.name ?: "",
                cityId = profile.city?.id?.toLong(),
                countryId = profile.country?.id?.toLong()
            ),
            holidayProduct = mapProductModel(
                model = profile.holidayProduct,
                price = price
            ),
    ...
    private fun isProfileFilled(profile: UserProfileDto): Boolean {
        return !profile.name.isNullOrEmpty()
            && profile.birthday != null
            && profile.country?.id != null
            && profile.country.id != 0
    ...
    private fun mapVehicles(vehicles: List<VehicleDto>?): List<UserVehicleModel> {
    ...
    private fun mapGroup(groups: List<GroupDto>?) = groups?.map { group ->
        GroupModel(
    ...
    private fun mapPhoto(photos: List<PhotoDto>?) = photos?.map {
        PhotoModel(
            id = it.id,
            link = it.link,
            isAdult = (it.isAdult == 1) && featureTogglesContainer.is18plusProfileFeatureToggle.isEnabled
        )
    } ?: emptyList()

    private fun mapGift(models: List<GiftDto>?) = models?.map { gift ->
        GiftModel(
            id = gift.giftId,
            typeId = gift.typeId,
    ...
    private fun mapProductModel(model: ProductDto?, price: String): ProductHolidayModel? {
        model ?: return null
    ...
    private fun mapSettingsFlag(model: UserSettingsFlagsDto?): UserSettingsFlagsModel {
