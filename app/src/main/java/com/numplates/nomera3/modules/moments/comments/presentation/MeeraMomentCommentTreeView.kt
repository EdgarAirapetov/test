...
import com.meera.core.base.viewbinding.findRootView
 ...
 */
class MeeraMomentCommentTreeView(
    private val viewModelController: MomentCommentTreeModel,
    private val binding: MeeraMomentCommentsBottomSheetBinding,
    ...
    private val commentsAdapterCallback = object : ICommentsActionsCallback {
                    ...
                    MeeraReactionsStatisticsBottomDialogFragment.makeInstance(
                        ...
                        when (destination) {
                            is MeeraReactionsStatisticsBottomDialogFragment.DestinationTransition.UserProfileDestination -> {
                                openUserProfile((destination.userEntity.userId))
                            }
                        }
        ...
        override fun onCommentLinkClick(url: String?) {
            val act = fragment.activity as? MeeraAct ?: return
            act.emitDeeplinkCall(url)
        }
        ...
        override fun onCommentMention(userId: Long) {
            val activity = fragment.activity as? MeeraAct ?: return
            findRootView(activity).findNavController().safeNavigate(
                resId = R.id.action_meeraMomentCommentsBottomSheetFragment_to_userInfoFragment,
                bundle = bundleOf(
        ...
        private fun openUserProfile(userId: Long) {
            fragment.findNavController().safeNavigate(
                R.id.action_global_userInfoFragment, bundleOf(
                    IArgContainer.ARG_USER_ID to userId,
                    IArgContainer.ARG_TRANSIT_FROM to AmplitudePropertyWhere.MOMENT.property
                )
            )
