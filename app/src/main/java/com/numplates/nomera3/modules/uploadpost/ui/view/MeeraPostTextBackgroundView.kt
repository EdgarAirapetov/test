...
import com.numplates.nomera3.databinding.MeeraViewPostTextBackgroundBinding
...
class MeeraPostTextBackgroundView @JvmOverloads constructor(
    context: Context,
    ...
    private val BACKGROUND_CORNERS_VALUE = 12.dp.toFloat()

    private var postBackground: PostBackgroundItemUiModel? = null

    private var showDefaultInput: (() -> Unit)? = null
    ...
    private var textChangedListener: ((CharSequence?, Int, Int, Int) -> Unit)? = null
    private var glideRequestListener : RequestListener<Drawable>? = null

    private val binding = LayoutInflater.from(context)
        .inflate(R.layout.meera_view_post_text_background, this, false)
        .apply(::addView)
        .let(MeeraViewPostTextBackgroundBinding::bind)
    ...
    fun getTextView(): MeeraTextViewWithImages {
    ...
    fun clearResources() {
        with(binding) {
            tvPostTextBackground.clearResources()
            etPostTextBackground.clearResources()
            textWatcher?.let {
                etPostTextBackground.removeTextChangedListener(it)
                textWatcher = null
            }
            textChangedListener = null
            ivPostTextBackground.apply {
                glideClear()
                setImageDrawable(null)
            }
            onTextChanged = null
            showDefaultInput = null
    ...
    private fun setupText(text: String, inputSelection: Int?) {
        ...
        binding.etPostTextBackground.apply {
            ...
            textChangedListener = fun(onChangedText: CharSequence?, _: Int, _: Int, _: Int) {
                        ...
                        binding.etPostTextBackground.post {
                            if (isEditable && isShowing()) {
                                if (isInputFullyVisible()) {
                                    ...
                                    this@MeeraPostTextBackgroundView.invisible()
                ...
                textWatcher = addTextChangedListener(onTextChanged = listener)
            }
    ...
    private fun renderBackground(backgroundUrl: String?) {
        if (!backgroundUrl.isNullOrEmpty()) {
            ...
            glideRequestListener = object : RequestListener<Drawable> {
            ...
            Glide.with(context)
                ...
                .transform(
                    GranularRoundedCorners(BACKGROUND_CORNERS_VALUE, BACKGROUND_CORNERS_VALUE, BACKGROUND_CORNERS_VALUE, BACKGROUND_CORNERS_VALUE),
                    FitCenter()
                )
    ...
    private fun getHighlightColor(background: PostBackgroundItemUiModel): Int =
        ContextCompat.getColor(context, if (background.isWhiteFont())
            R.color.uiKitColorAccentPrimary else R.color.uiKitColorForegroundAddNavy)
}
