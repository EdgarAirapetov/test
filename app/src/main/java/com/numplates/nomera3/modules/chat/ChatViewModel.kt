...
class ChatViewModel @Inject constructor(
    ...
    private val getMessageEvents: GetMessageEventsObserverUseCase,
    ...
    private val getMediaKeyboardStickersUseCase: GetMediaKeyboardStickersUseCase,
    ...
    fun handleAction(action: ChatActions) {
        ...
        when (action) {
            ...
            is ChatActions.AddToFavorites -> addToFavoritesHandleEvent(action.mediaPreview, action.mediaUrl, action.lottieUrl)
            ...
            is ChatActions.CopyMessageContent -> sendEffect(
                ChatMessageViewEvent.OnCopyMessageContent(
                    action.message,
                    action.messageView
                )
            )
            ...
            is ChatActions.ReplyMessage -> sendEffect(ChatMessageViewEvent.OnMessageReply(action.message))
            ...
            is ChatActions.SendFavoriteRecent -> {
                sendEffect(ChatMessageViewEvent.OnSendFavoriteRecent(action.favoriteRecentUiModel, action.type))
            }
            is ChatActions.SetupMediaPreview ->
                sendEffect(ChatMessageViewEvent.OnSetupMediaPreview(action.media, action.isMeeraMenu))
            ...
            is ChatActions.PlayVoiceMessages -> playVoiceMessages(action.message, action.position)
            is ChatActions.PlayMeeraVoiceMessage -> Unit
            ...
            is ChatActions.CopyImageMessageAttachment -> getMediaAttachmentForCopy(action.message, action.attachmentsIndex)
            ...
            is ChatActions.OnEditorOpen -> logEditorOpen(action.uri)
            is ChatActions.OnPhotoEdits -> action.nmrAmplitude?.let(::logPhotoEdits)
            is ChatActions.OnVideoEdits -> action.nmrAmplitude?.let(::logVideoEdits)
            is ChatActions.MapAttachmentsData -> Unit
        }
    }
    ...
    fun messageListFlow() = selectMessagesSource()
    ...
    fun addMessageMedias(
        ...
        if (isMaxCountCheck && mediaCount >= MAX_PICTURE_COUNT) {
            _liveMessageViewEvent.value = ChatMessageViewEvent.OnShowMaxSelectedMediaCountErrorMessage
            return
        }
        uris.map(Uri::toString).forEach(addPhotoFromClipboardUseCase::invoke)
    }
                    ...
                    success = { room ->
                        when (room?.approved) {
                            ...
                            DialogApproved.FORBIDDEN.key -> {
                                ...
                                liveChatRequestViewEvent.postValue(ChatRequestViewEvent.ForbidChatRequest(room))
                                _liveIsChatRequestMenuVisible.postValue(false)
    ...
    fun isLetRecordAudioMessageTooltipWasShownTimes() =
        tooltipsInteractor.isNeedShowRecordAudioMessageTip(DEFAULT_TIMES)

    // увеличиваем на 1 кол-во показов до тех пор пока кол-во не достигнет 3х
    fun incRecordAudioMessageTooltipWasShown() = tooltipsInteractor.recordAudioMessageTipWasShowed(DEFAULT_TIMES)

    fun shouldShowPhoneAbilityTooltip() = tooltipsInteractor.isNeedShowPhoneAbilityTip(DEFAULT_TIMES)

    fun isReleaseButtonSentAudioMessageTooltipWasShownTimes() =
        tooltipsInteractor.isNeedShowReleaseButtonSentAudioMessageTip(DEFAULT_TIMES)

    fun incReleaseButtonSentAudioMessageTooltipWasShown() =
        tooltipsInteractor.releaseButtonSentAudioMessageTipWasShowed(DEFAULT_TIMES)
    ...
    private fun logEditorOpen(uri: Uri) = viewModelScope.launch {
        amplitudeEditor.editorOpenAction(
            where = AmplitudePropertyWhere.CHAT,
            type = when (fileManager.getMediaType(uri)) {
     ...
     */
    fun updateMessageReadDb(roomId: Long?, message: MessageEntity) {
        roomId?.let { id ->
            viewModelScope.launch(Dispatchers.IO) {
                ...
                if (isInternetConnected()) {
                    trySendReadStatus(roomId, message)
                } else {
                    chatWorkManagerDelegate.sendReadMessageDoWork(id, message)
                }
            }
        }
    }

    private suspend fun trySendReadStatus(roomId: Long, message: MessageEntity) {
        val isSuccessReadStatus = messagesInteractor.readMessageNetwork(
            roomId = roomId,
            messageIds = mutableListOf(message.msgId)
        )
        if (isSuccessReadStatus) {
            Timber.d("Message read status successfully send MessageID:${message.msgId}")
        } else {
            Timber.e("Message read status fail send MessageID:${message.msgId} Attempt send through worker")
            chatWorkManagerDelegate.sendReadMessageDoWork(roomId, message)
        }
    }
    ...
    private fun enableChat(companionUid: Long) {
        viewModelScope.launch {
            runCatching {
                ...
                Timber.tag(this@ChatViewModel.simpleName).e(it)
            }
        }
    }
    ...
    private fun playVoiceMessages(currentMessage: MessageEntity?, startAdapterPosition: Int) {
        viewModelScope.launch {
            currentMessage?.let { message ->
                val messagesToPlay = getMessagesToPlay(message)
                val playMessagesQueue = getMessagesQueueToPlay(messagesToPlay, startAdapterPosition)
                _liveMessageViewEvent.postValue(
                    ChatMessageViewEvent.OnPlayVoiceMessage(startAdapterPosition, playMessagesQueue)
                )
            }
        }
    }

    private suspend fun getMessagesToPlay(message: MessageEntity): List<MessageEntity> {
        val messagesToPlay = mutableListOf<MessageEntity>()
        messagesToPlay.add(message)
        val nextMessages = messagesInteractor.getNextMessages(
            roomId = message.roomId,
            createdAt = message.createdAt
        )
        nextMessages.forEach { msg ->
            if (msg?.itemType != ITEM_TYPE_AUDIO_SEND && msg?.itemType != ITEM_TYPE_AUDIO_RECEIVE) return@forEach
            messagesToPlay.add(msg)
        }
        return messagesToPlay
    }

    private fun getMessagesQueueToPlay(
        messages: List<MessageEntity>,
        startAdapterPosition: Int
    ): LinkedList<PlayMessageDataUiModel> {
        val playMessagesQueue = LinkedList<PlayMessageDataUiModel>()
        messages.forEachIndexed { index, messageEntity ->
            playMessagesQueue.add(
                PlayMessageDataUiModel(
            ...
            viewModelScope.launch(Dispatchers.IO) {
                runCatching {
                    ...
                    if (rooms.isNotEmpty() && currentRoom != null) {
                        _liveMessageViewEvent.postValue(ChatMessageViewEvent.OnRefreshRoomData(currentRoom))
    ...
    fun scrollToMessage(message: MessageEntity) {
        sendEffect(ChatMessageViewEvent.OnScrollToMessage(message.msgId))
    }

    fun scrollToReplyParentMessage(childMessage: MessageEntity) {
        viewModelScope.launch {
            runCatching {
                getMessageByIdUsecase.invoke(childMessage.parentMessage?.parentId ?: return@launch)
            }
                .onFailure(Timber::e)
                .onSuccess { message -> scrollToMessage(message ?: return@launch) }
        }
    }
        ...
        roomId?.let { _ ->
            viewModelScope.launch {
                runCatching {
                    ...
                    _liveMessageViewEvent.postValue(ChatMessageViewEvent.OnSetUnsentMessages(unsentMessageCount))
                }
            }
    ...
    private fun mediaKeyboardFavoriteRecentLongClicked(
        ...
        val mediaUiModel = MediaUiModel.fromMediakeyboardFavoriteRecentUiModel(favoriteRecent, dialogEntity?.roomId)
        val mediaPreviewUiModel = MediaPreviewUiModel(
            media = mediaUiModel,
            type = type,
        ...
        emitMediaKeyboardEvent(ChatMediaKeyboardEvent.ShowMediaPreview(
            mediaPreview = mediaPreviewUiModel,
    ...
    fun setNoMediaPlaceholderMessage(message: MessageEntity) {
        viewModelScope.launch {
    ...
    fun disableChatRequestImageBlur(message: MessageEntity?) {
        viewModelScope.launch(Dispatchers.IO) {
    ...
    private fun loadMoreFavorites(startId: Int? = null) {
        viewModelScope.launch {
            val favorites = runCatching { loadFavoritesUseCase.invoke(startId) }.onFailure { Timber.e(it) }
    ...
    private fun loadGreetingSticker() {
        viewModelScope.launch {
            ...
            _liveMessageViewEvent.value = ChatMessageViewEvent.OnGreetingStickerFound(greetingSticker)
        }
    }
    ...
    private fun getAndSetupStickers() {
        ...
        getStickersJob = viewModelScope.launch {
            runCatching {
                val stickers = getMediaKeyboardStickersUseCase.invoke()
                val stickerPacks = stickers.stickerPacks.map(stickersMapper::mapStickerPackDomainToUiModel)
                val recentStickers = stickers.recentStickers.map(mediakeyboardFavoritesMapper::mapDomainToUiModel)
                emitMediaKeyboardEvent(ChatMediaKeyboardEvent.OnStickersLoaded(stickerPacks, recentStickers))
    ...
    private fun handleIncomeMessageBeforeSaveDb(message: MessageEntity) {
        ...
        viewModelScope.launch {
            when  {
                message.isMy() || message.isDeletedOrTypeEvent() || message.isEdited() || message.isMomentDeleted()
                    -> insertMessageIntoDb(message)
                message.isUnreadYet() || message.isRecognizedVoiceMessage()
                    -> saveMessageToUniqueSetAndSaveIntoDb(message)
                else -> { Timber.e("It message [${message.msgId}] have other condition") }
            }
        }
    }
    ...
    private fun List<MessageEntity>.withInsertedDateDividers(): List<MessageEntity> {
    ...
    private fun getMediaAttachmentForCopy(message: MessageEntity, attachmentsIndex: Int){
        viewModelScope.launch {
            runCatching {
                ...
                if (attachment != null) {
                    _liveMessageViewEvent.value = ChatMessageViewEvent.OnCopyAttachmentImageMessage(attachment)
                } else {
