...
class MessengerShareCommunityViewHolder(
    itemView: View,
    ...
    init {
        val listeners =
            listenersMapper.mapShareCommunityListeners(::getMessage) as MessageWrapperUiListeners.ShareCommunityListeners
    ...
    override fun bind(item: ChatMessageDataUiModel) {
        super.bind(item)
        profileView.setConfig((item.messageConfig as MessageConfigWrapperUiModel.ShareCommunity).config)
    }
}
