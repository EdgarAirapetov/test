...
class ProfileStatisticsViewsFragment :
    ...
    private fun initViews() {
        ...
        binding?.apply {
            ...
            if (slide.count != null && slide.growth != null) {
                setupViewsText(slide.count, slide.trend)
            }
        }
    }

    private fun setupViewsText(count: Long, trend: ProfileStatisticsTrend) {
        ...
        when (trend) {
            ProfileStatisticsTrend.POSITIVE -> {
                ...
                val arrowColor = ContextCompat.getColor(requireContext(), R.color.profile_statistics_green)
            ...
            ProfileStatisticsTrend.NEGATIVE -> {
                ...
                val spannableText = SpannableString("$formattedCount $arrow")
                val arrowColor = ContextCompat.getColor(requireContext(), R.color.profile_statistics_red)
                val colorSpan = ForegroundColorSpan(arrowColor)
                spannableText.setSpan(
                    colorSpan,
                    spannableText.length - 1,
    ...
    companion object {
        ...
        fun newInstance(slideResponse: SlideModel): ProfileStatisticsViewsFragment {
            return ProfileStatisticsViewsFragment().apply {
