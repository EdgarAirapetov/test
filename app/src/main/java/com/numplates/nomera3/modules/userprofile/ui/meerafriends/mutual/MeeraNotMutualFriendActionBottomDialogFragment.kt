...
import com.numplates.nomera3.databinding.MeeraFragmentNotMutualFriendActionBottomDialogBinding
import com.numplates.nomera3.modules.bump.ui.viewmodel.ShakeBottomDialogViewModel
import com.numplates.nomera3.presentation.router.IArgContainer

class MeeraNotMutualFriendActionBottomDialogFragment :
    UiKitBottomSheetDialog<MeeraFragmentNotMutualFriendActionBottomDialogBinding>() {

    private val argUserId by lazy { requireArguments().getLong(IArgContainer.ARG_USER_ID) }
    override val inflateContent: (LayoutInflater, ViewGroup?, Boolean) -> MeeraFragmentNotMutualFriendActionBottomDialogBinding
        get() = MeeraFragmentNotMutualFriendActionBottomDialogBinding::inflate
    ...
    private fun initListeners() {
        ...
        contentBinding?.cellSubscribe?.setThrottledClickListener {
            setFragmentResult(
                KEY_NOT_MUTUAL_FRIEND_RESULT, bundleOf(
                    KEY_NOT_MUTUAL_FRIEND_ACTION to KEY_NOT_MUTUAL_FRIEND_SUBSCRIBE,
                    IArgContainer.ARG_USER_ID to argUserId
                )
            )
            dismiss()
        }
        contentBinding?.cellAddToFriend?.setThrottledClickListener {
            setFragmentResult(
                KEY_NOT_MUTUAL_FRIEND_RESULT, bundleOf(
                    KEY_NOT_MUTUAL_FRIEND_ACTION to KEY_NOT_MUTUAL_FRIEND_ADD_TO_FRIEND,
                    IArgContainer.ARG_USER_ID to argUserId
                )
            )
            dismiss()
    ...
    companion object {
        const val NOT_MUTUAL_FRIEND_ACTION_DIALOG = "notMutualFriendActionDialog"
        const val KEY_NOT_MUTUAL_FRIEND_RESULT = "notMutualFriendResult"
        const val KEY_NOT_MUTUAL_FRIEND_ACTION = "notMutualFriendAction"
        const val KEY_NOT_MUTUAL_FRIEND_SUBSCRIBE = "notMutualFriendSubscribe"
        const val KEY_NOT_MUTUAL_FRIEND_ADD_TO_FRIEND = "notMutualFriendAddToFriend"


        @JvmStatic
        fun show(
            fragmentManager: FragmentManager, userId: Long
        ): MeeraNotMutualFriendActionBottomDialogFragment {
            val instance = MeeraNotMutualFriendActionBottomDialogFragment()
            instance.arguments = bundleOf(IArgContainer.ARG_USER_ID to userId)
            instance.show(
                fragmentManager, NOT_MUTUAL_FRIEND_ACTION_DIALOG
            )
            return instance
        }
    }
