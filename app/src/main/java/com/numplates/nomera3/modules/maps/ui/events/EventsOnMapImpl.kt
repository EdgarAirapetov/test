...
class EventsOnMapImpl @Inject constructor(
    ...
    private val eventConfigurationUiModeFlow = MutableStateFlow(EventConfigurationUiMode.CLOSED)
    ...
    private val selectedEventTypeFlow = MutableStateFlow(EventType.getDefault())
    private val selectedDateFlow = MutableStateFlow<LocalDate?>(null)
    private val selectedTimeFlow = MutableStateFlow<LocalTime?>(null)
    ...
    init {
        ...
        eventConfigurationUiModeFlow
            .onEach { eventConfigurationUiMode ->
                val visibility = if (eventConfigurationUiMode == EventConfigurationUiMode.CLOSED) {
    ...
    override fun setupEventEditing(setupUiModel: EventEditingSetupUiModel) {
        ...
        eventConfigurationUiModeFlow.value = EventConfigurationUiMode.OPEN
    }
    ...
    override fun onEventPublished() {
        setEventConfigurationUiMode(EventConfigurationUiMode.CLOSED)
    ...
    override fun setEventConfigurationUiMode(eventConfigurationUiMode: EventConfigurationUiMode) {
        when {
            eventConfigurationUiMode == EventConfigurationUiMode.OPEN &&
                eventConfigurationUiModeFlow.value == EventConfigurationUiMode.CLOSED -> {
            ...
            eventConfigurationUiMode == EventConfigurationUiMode.OPEN &&
                eventConfigurationUiModeFlow.value == EventConfigurationUiMode.ONBOARDING -> {
                scope.launch {
                    runCatching {
    ...
    override fun setEventConfigurationUiMode(eventConfigurationUiMode: MeeraEventConfigurationUiMode) = Unit
    ...
    //TODO: remove
    override fun setSelectedTimeWithRes(time: LocalTime): LocalTime = LocalTime.now()

    override fun setSelectedTime(time: LocalTime) {
        selectedTimeFlow.value = time
        configurationParamsAnalyticsModel = configurationParamsAnalyticsModel.copy(
        ...
        if (userEventCountCheckInProgressFlow.compareAndSet(expect = false, update = true)) {
            ...
            addEventJob = scope.launch {
                try {
                    ...
                    this@EventsOnMapImpl.availableEventCount = availableEventCount
    ...
    override fun setSelectedEvent(event: PostUIEntity) = Unit
    ...
    override fun onSelectedImage(pathPhoto: UIAttachmentPostModel) {
