...
import com.numplates.nomera3.databinding.ViewMeeraMediaAttachmentViewPagerBinding
import com.numplates.nomera3.modules.uploadpost.ui.AttachmentMediaActions
import com.numplates.nomera3.modules.uploadpost.ui.MeeraAttachmentMediaAdapter
...
private const val SHOW_PAGER_INDICATORS_DELAY = 50L

class MeeraMediaAttachmentViewPager @JvmOverloads constructor(
    context: Context,
    ...
    private val binding = LayoutInflater.from(context)
        .inflate(R.layout.view_meera_media_attachment_view_pager, this, false)
        .apply(::addView)
        .let(ViewMeeraMediaAttachmentViewPagerBinding::bind)

    private var attachmentMediaActions: AttachmentMediaActions? = null

    private val attachmentMediaAdapter: MeeraAttachmentMediaAdapter by lazy(LazyThreadSafetyMode.NONE) {
        MeeraAttachmentMediaAdapter(imagePositioningListener = { imagePositioningInProcess ->
    ...
    private fun initClickListeners() {
        binding.flEditAttachment.setThrottledClickListener {
            attachmentMediaActions?.onItemEditClick(currentPagerPosition)
        }

        binding.flDeleteAttachment.setOnClickListener {
            attachmentMediaActions?.onItemCloseClick(currentPagerPosition)
        }

        binding.flOpenEditorStickers.setThrottledClickListener {
        ...
        getCurrentAttachmentItem()?.let {
            when (it.type) {
                AttachmentPostType.ATTACHMENT_GIF -> {
                    binding.flDeleteAttachment.visible()
                    binding.flEditAttachment.gone()
                    binding.flOpenEditorStickers.gone()
                }
                ...
                AttachmentPostType.ATTACHMENT_PREVIEW -> {
                    ...
                    binding.flDeleteAttachment.visible()
                    binding.flOpenEditorStickers.visible()
                    binding.flEditAttachment.visible()
