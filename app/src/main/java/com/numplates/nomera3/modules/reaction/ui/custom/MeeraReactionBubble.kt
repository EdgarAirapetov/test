...
import com.numplates.nomera3.databinding.MeeraReactionBubbleBinding
...
private val INCREASE_SIZE_VERTICAL_OFFSET_DP = 4.dp

@SuppressLint("ViewConstructor")
class MeeraReactionBubble private constructor(
    ...
    private val binding: MeeraReactionBubbleBinding =
        MeeraReactionBubbleBinding.inflate(LayoutInflater.from(context), this, true)
    ...
    companion object {
        ...
        fun show(
            ...
            val bubbleInViewTree = container.children.find { it is MeeraReactionBubble } as? MeeraReactionBubble
            bubbleInViewTree?.setBubbleLayoutParams(position, container)
            ...
            val bubble = MeeraReactionBubble(
                container = container,
                showMorningEvening = showMorningEvening,
                selectListener = selectListener,
                hideListener = hideListener
            ).apply {
                setBubbleLayoutParams(position, container)
        ...
        private fun MeeraReactionBubble.setBubbleColor(
            contentActionBarType: MeeraContentActionBar.ContentActionBarType
        ) {
        ...
        private fun MeeraReactionBubble.setBubbleLayoutParams(position: Point, container: ViewGroup) {
            val containerLocation = IntArray(2).also {
                container.getLocationOnScreen(it)
            }
            val containerOffset = containerLocation[1]

            layoutParams = LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
            ).apply {
                setMargins(position.x, position.y - containerOffset, 0, 0)
            }
        }

        private fun MeeraReactionBubble.setMoveUpAnimationEnabled(isMoveUpAnimationEnabled: Boolean) {
