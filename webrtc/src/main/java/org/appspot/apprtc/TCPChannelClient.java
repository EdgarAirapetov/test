...
/**
 * Replacement for WebSocketChannelClient for direct communication between two IP addresses. Handles the signaling
 * between the two clients using a TCP connection.
 * <p>
 * All public methods should be called from a looper executor thread passed in a constructor, otherwise exception will
 * be thrown. All events are dispatched on the same thread.
 */
public class TCPChannelClient {
    ...
    /**
     * Initializes the TCPChannelClient. If IP is a local IP address, starts a listening server on that IP. If not,
     * instead connects to the IP.
     *
    ...
    /**
     * Callback interface for messages delivered on TCP Connection. All callbacks are invoked from the looper executor
     * thread.
     */
    ...
    /**
     * Base class for server and client sockets. Contains a listening thread that will call eventListener.onTCPMessage
     * on new messages.
     */
